...
test('fetchConfigFromReq - basic', (t) => {
  ...
  const { fetchUrl, fetchParams } = fetchConfigFromReq({ network, req, source: 'eth-json-rpc-backbone', projectId })
...
test('createBackboneMiddleware throws with an invalid projectId', (t) => {
  ...
  t.throws(() => createBackboneMiddleware({ projectId: 42 }), /Invalid value for 'projectId'/u)
  t.throws(() => createBackboneMiddleware({ projectId: null }), /Invalid value for 'projectId'/u)
  t.throws(() => createBackboneMiddleware({ projectId: undefined }), /Invalid value for 'projectId'/u)
  t.throws(() => createBackboneMiddleware({ projectId: '' }), /Invalid value for 'projectId'/u)
  t.end()
...
test('createBackboneMiddleware throws with invalid headers', (t) => {
  ...
  t.throws(() => createBackboneMiddleware({ projectId: 'foo', headers: null }), /Invalid value for 'headers'/u)
  t.throws(() => createBackboneMiddleware({ projectId: 'foo', headers: 42 }), /Invalid value for 'headers'/u)
  t.throws(() => createBackboneMiddleware({ projectId: 'foo', headers: '' }), /Invalid value for 'headers'/u)
  t.end()
